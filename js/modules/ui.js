...
export function render() {
    ...
    list.innerHTML = paged.map(r => {
        ...
        return '<div class="card longpress-card fade-in' + (isA ? '' : ' inactive') + '" data-id="' + r.id + '" style="display:flex;align-items:flex-start;gap:12px;cursor:pointer" onclick="if(event.target.closest(\'.card-actions\')||event.target.classList.contains(\'sel-check\')){}else if(typeof selectMode!==\'undefined\'&&selectMode){const cb=this.querySelector(\'.sel-check\');if(cb){cb.checked=!cb.checked;toggleSelectItem(\'' + r.id + '\',cb);}}else{editResident(\'' + r.id + '\');}" >' + (selectMode ? '<input type="checkbox" class="sel-check item-check" data-id="' + r.id + '" ' + (selectedIds.has(r.id) ? 'checked' : '') + ' onchange="toggleSelectItem(\'' + r.id + '\',this)" style="margin-top:10px;flex-shrink:0">' : '') + '<div class="avatar" style="background:' + avatarBg + ';margin-top:2px">' + esc(initial) + '</div><div style="flex:1;min-width:0"><div class="card-top"><div>' +
